...
class App extends Component {
  ...
  part1 = () => {
    var pointsWire1 = this.getPointsB(this.wire5)
    var pointsWire2 = this.getPointsB(this.wire6)
    var intersections = this.findIntersectionsB(pointsWire1, pointsWire2);
    /*console.log(pointsWire1)
    console.log(pointsWire2)
    console.log(intersections)*/
    //var intersections = this.findIntersections(pointsWire1, pointsWire2);
    var distances = intersections.map(x => this.distance(this.central_point, x))
    return Math.min.apply(Math, distances);
  }

  part2 = () => {
    var pointsWire1 = this.getPointsB(this.wire1)
    var pointsWire2 = this.getPointsB(this.wire2)
    var intersections = this.findIntersectionsWithSteps(pointsWire1, pointsWire2);
    var distances = intersections.map(x => x.steps)
    return Math.min.apply(Math, distances)
  }
  ...
  getPointsB = (array) => {
    ...
    for(var i = 0; i < array.length; i++){
      let elem = array[i];
      let movCode = elem[0];
      let movSign = this.getSign(movCode);
      elem = elem.slice(1, elem.length)
      let spaces = parseInt(elem);
      if (this.isXAxis(movCode)){
        pointX += (movSign*spaces)
      }else if (this.isYAxis(movCode)){
        pointY += (movSign*spaces)
      }
  ...
  findIntersectionsB = (pointsA, pointsB) => {
    let intersections = [];
    for(var i = 0; i < pointsA.length-1; i++){
      let recta1 = {
        punto1: pointsA[i],
        punto2: pointsA[i+1],
        isVertical: pointsA[i].x - pointsA[i+1].x === 0
      }
      //console.log(recta1)
      ...
      for(var j = 0; j < pointsB.length-1; j++){
        let recta2 = {
          punto1: pointsB[j],
          punto2: pointsB[j+1],
          isVertical: pointsB[j].x - pointsB[j+1].x === 0
        }
        //console.log(recta2)
        if (recta1.isVertical && recta2.isVertical && recta1.punto1.x === recta2.punto2.x){
          //minimo del segundo es menor que el maximo del primero
          //solapamiento

        } else if (recta1.isVertical && !recta2.isVertical){
          //intersections.push(this.getValues(recta1, recta2));
          let val = this.getValues(recta1, recta2);
          if (val.length > 0) intersections.push(val);
        } else if (!recta1.isVertical && recta2.isVertical){
          let val = this.getValues(recta2, recta1);
          if (val.length > 0) intersections.push(val);
        } else if (!recta1.isVertical && !recta2.isVertical && recta1.punto1.x === recta2.punto2.x){
  ...
  findIntersectionsWithSteps = (pointsA, pointsB) => {
    let intersections = [];
    let stepsRecta1 = 0;
    for(var i = 0; i < pointsA.length-1; i++){
      let recta1 = {
      ...
      stepsRecta1 += Math.abs(recta1.isVertical ? 
        recta1.punto1.y - recta1.punto2.y : 
        recta1.punto1.x - recta1.punto2.x)
      //console.log(recta1)
      let stepsRecta2 = 0;
      for(var j = 0; j < pointsB.length-1; j++){
        let recta2 = {
        ...
        stepsRecta2 += Math.abs(recta2.isVertical ? 
          recta2.punto1.y - recta2.punto2.y : 
          recta2.punto1.x - recta2.punto2.x)
        //console.log(recta2)
        if (recta1.isVertical && recta2.isVertical && recta1.punto1.x === recta2.punto2.x){
          //minimo del segundo es menor que el maximo del primero
          //solapamiento
          ...
          let val = this.getValues(recta1, recta2);
          if (val.length > 0) {
            let steps = stepsRecta1 + stepsRecta2
            intersections.push({point: val, steps: stepsRecta1 + stepsRecta2 - val[2]});
          }
        } else if (!recta1.isVertical && recta2.isVertical){
          let val = this.getValues(recta2, recta1);
          if (val.length > 0) {
  ...
  getValues = (recta1, recta2) => {
    if (Math.min(recta1.punto1.y, recta1.punto2.y) <= recta2.punto1.y && 
        Math.max(recta1.punto1.y, recta1.punto2.y) >= recta2.punto1.y &&
        Math.min(recta2.punto1.x, recta2.punto2.x) <= recta1.punto1.x && 
        Math.max(recta2.punto1.x, recta2.punto2.x) >= recta1.punto1.x )
        {
          let point = {x: recta1.punto1.x, y: recta2.punto1.y}
          let steps = 
            (Math.max(recta1.punto1.y, recta1.punto2.y) - point.y) + 
            (Math.max(recta2.punto1.x, recta2.punto2.x) - point.x)
          return [
            recta1.punto1.x, 
            recta2.punto1.y,
            steps
  ...
  isXAxis = (code) => code === 'L' || code === 'R'
  isYAxis = (code) => code === 'D' || code === 'U'
