...
export default class Day5 extends Component {
  ...
  hasTwoParameters = (opCode) => opCode === 5 || opCode === 6;

  hasThreeParameters = (opCode) => opCode === 1 || opCode === 2 || opCode === 7 || opCode === 8;

  isInmediateMode = (param) => param === 1;

  isPositionMode = (param) => param === 0;

  getOpCode = (strElem) => parseInt(strElem.substr(strElem.length - 2));
  ...
  render() {
    ...
    const part1 = this.part1(input1)
    return (
      <div>
        <h2>Problem of day 5. </h2>
        <p>
          Part 1 solution: {part1[part1.length-1]}
        </p>
        <p>
          Part 2 solution: {this.part1(input2)}
        </p>
      </div>
    );
  }
