...
class App extends Component {
  ...
  part1 = () => {
    //var pointsWire1 = this.getPoints(this.wire1)
    //var pointsWire2 = this.getPoints(this.wire2)
  ...
  findIntersections = (pointsA, pointsB) => {
    let intersections = []
    for(var i = 0; i < pointsA.length; i++){
      for(var j = 0; j < pointsB.length; j++){
        if (pointsA[i][0] === pointsB[j][0] &&
            pointsA[i][1] === pointsB[j][1])
        {
          intersections.push(pointsA[i]);
        }
      }
    }
